...
public class PermSystem {
	...
	public static final void setGroupPEX(final Player p) {
		...
		final String[] groups = { Main.config.getString("PermissionSystem.PermissionsEx.BuildGroup") };
		user.setGroups(groups);
		onGroupChange(p);
		p.sendMessage(ChatColor.GREEN + "You have been moved to the build group!");
		Main.INFO("Set " + p.getName() + "s group to the one specifyed in the config.yml");
	...
	public static final void setGroupGM(final Player p) {
		...
		final Plugin gm = Main.pm.getPlugin("GroupManager");
		final OverloadedWorldHolder handler = ((GroupManager) gm).getWorldsHolder().getWorldData(p);
		if (handler == null) {
			Main.WARNING("Failed to move " + p.getName() + " to the build group!, GroupManager");
			return;
		}
		final org.anjocaido.groupmanager.data.Group group = handler.getGroup(Main.config.getString("PermissionSystem.GroupManager.BuildGroup"));
	...
	public static final void setGroupBP(final Player p) {
		...
		ApiLayer.setGroup(p.getWorld().getName(), CalculableType.USER, p.getName(), Main.config.getString("PermissionSystem.bPermissions.BuildGroup"));
		ApiLayer.update();
		onGroupChange(p);
		p.sendMessage(ChatColor.GREEN + "You have been moved to the build group!");
		Main.INFO("Set " + p.getName() + "s group to the one specifyed in the config.yml");
